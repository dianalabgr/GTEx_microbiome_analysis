...
def buildTaxonomy(nodesFile, namesFile, taxid_Accessions):
	...
	for key in taxid_Accessions:
		...
		parent = key
		cnt = 0
		while parent not in ['131567', '10239', '157619', '623144', '1']:
...
def count_taxid(i,taxid_Accessions,accession_Readcounts):
	taxid_Readcounts=dict()
	for key in taxid_Accessions:
		if accession_Readcounts[i][0] in taxid_Accessions[key]:
			taxid_Readcounts[key] = accession_Readcounts[i][1]
	return taxid_Readcounts	

def counts2lineage(accession_Readcounts, taxid_Accessions, lineage, threads):
	lineage_Readcounts = list()
	accession_Readcounts_notZero=[x for x in accession_Readcounts if x[1]!=0]
	taxid_Readcounts = dict()
	list_taxid_Readcounts=list()
	partial_count_taxid = partial(count_taxid, taxid_Accessions = taxid_Accessions,accession_Readcounts = accession_Readcounts_notZero)
	with mp.Pool(threads) as pool:
		list_taxid_Readcounts=pool.map(partial_count_taxid, range(len(accession_Readcounts_notZero)))
	for taxid_Readcounts_small in list_taxid_Readcounts:
		key_one=list(taxid_Readcounts_small.keys())[0]
		value_one=taxid_Readcounts_small[key_one]
		if key_one in taxid_Readcounts.keys():
			taxid_Readcounts[key_one] += value_one
		else :
			taxid_Readcounts[key_one] = value_one
	for key in taxid_Accessions:
		if key in taxid_Readcounts:
			...
			lineage_Readcounts.append([lineage[key], "0.0"])
...
if __name__ == '__main__':
	...
	parser.add_argument('-threads', '-t', help = 'Number of threads', default = 6)
	...
	_lineage_Readcounts = counts2lineage(_accession_Readcounts, _taxid_Accessions, _lineage, args.threads)
