...
dim(all_sub2)
summary(metadata3$body_mass_index)
metadata3$body_mass_index
...
for (tissue in tissues) {
  ...
  if (length(metadata_tissue$samples.submitter_id)<20) {
    cumulative_results=rbind(cumulative_results,data.frame("tissue"=tissue, "factor"="bmi", 
  ...
  for (i in seq(1,100,by=1)) {
    ...
    summary(metadata_test$body_mass_index)
    dim(all_sub4)
  ...
  write.csv(permutation_results,paste("./permutations_bmi/CSS_core_",sub(" ","_",tissue),"_","bmi",".csv",sep=""))
  temp=data.frame(tissue, "bmi", mean(unlist(permutation_results$RMSE)), mean(unlist(permutation_results$Rsquared)), mean(unlist(permutation_results$MAE)))
  colnames(temp) = c("tissue","factor","RMSE","Rsquared","MAE")
  cumulative_results=rbind(cumulative_results, temp)
  }
}
cumulative_results=cumulative_results[2:9,]
write.csv(cumulative_results,file=paste("./CSS_total_tissues","_","bmi",".csv",sep=""))
