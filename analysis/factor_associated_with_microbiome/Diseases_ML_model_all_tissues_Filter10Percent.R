...
dim(metadata3)
all_sub3=all_sub[,metadata3$samples.submitter_id]
dim(all_sub3)

#################################################################################################################################################
################################################ Keep the wanted disease and the healthy samples #######################################################################################
diseases_studied=c("hypertension_history", "ischemic_heart_history", "diabetes_ii_history", "diabetes_i_history", "arthritis_history",
                   "seizures_history", "schizophrenia_history", "rheumatoid_arthritis_history", "liver_disease_history", "dialysis_treatment",
                   "depression_history", "copd_clrd_history", "cerebrovascular_disease_history", "asthma_history", "alzheimer_or_dementia_history")

for (disease in diseases_studied) {
  print(disease)
  file=paste("./CSS_total_tissues","_",disease,".csv",sep="")
  if (file.exists(file)) {
    next
  } else {
...
colnames(cumulative_results)=c("tissue","disease","ROC","margin_of_error_AUROC", "PR",
...
for (tissue in tissues) {
  ...
  dim(metadata_tissue)
  if ("Unknown"%in%unique(metadata_tissue[,eval(disease)])) {
    metadata_tissue=metadata_tissue[-which(metadata_tissue[,eval(disease)]=="Unknown"),]
  }
  table(metadata_tissue[,eval(disease)])

  all_sub4=all_sub3[,metadata_tissue$samples.submitter_id]
  dim(all_sub4)
  ...
  if (length(metadata_tissue$samples.submitter_id)<20 | length(metadata_tissue[which(metadata_tissue[,eval(disease)]=="Yes"),1]) <20) {
    cumulative_results=rbind(cumulative_results,data.frame("tissue"=tissue, "disease"=eval(disease), 
 ...
 # index <- createDataPartition(metadata4$hypertension_history, p = 0.8, list = FALSE)
  index <- createDataPartition(metadata4[,eval(disease)], p = 0.7, list = FALSE)
  ...
  TypeComparison <- metadata_train[,eval(disease)]
  TypeString = "Yes"
  TypeComparisonFactor <- factor(ifelse(TypeComparison == TypeString, yes = "Disease", no = "Healthy"),
  ...
  TypeComparison <- metadata_test[,eval(disease)]
  TypeString = "Yes"
  TypeComparisonFactor <- factor(ifelse(TypeComparison == TypeString, yes = "Disease", no = "Healthy"),
                                 levels = c("Disease", "Healthy"))
  ...
  refactoredTrainY <- relevel(refactoredTrainY, ref = gsub('([[:punct:]])|\\s+','',"Disease"))
  refactoredTestY <- relevel(refactoredTestY, ref = gsub('([[:punct:]])|\\s+','',"Disease"))
  ...
  predProbs <- as.numeric(predict(mlModel, newdata = testX, type = "prob")[, gsub('([[:punct:]])|\\s+','',"Disease")])
  fg <- predProbs[refactoredTestY == gsub('([[:punct:]])|\\s+','',"Disease")]
  bg <- predProbs[refactoredTestY == "Healthy"]
  ...
  write.csv(permutation_results,paste("./permutations/CSS_core_",sub(" ","_",tissue),"_",disease,".csv",sep=""))
  ...
  temp=data.frame(tissue, disease, mean(unlist(permutation_results$AUROC)), margin_of_error_X1,mean(unlist(permutation_results$AUPR)), margin_of_error_X2, mean(unlist(permutation_results$AUPR_random)))
  colnames(temp) = c("tissue","disease","ROC","margin_of_error_AUROC", "PR","margin_of_error_AUPR","PR_random")
  cumulative_results=rbind(cumulative_results, temp)
  }

}
cumulative_results2=cumulative_results[2:9,]
write.csv(cumulative_results,file=paste("./CSS_total_tissues","_",disease,".csv",sep=""))
}
}
