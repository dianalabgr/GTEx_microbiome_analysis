...
table(metadata3$ancestry)
metadata3=metadata3[which(metadata3$ancestry %in%c("['White']","['Black or African American']")),]
all_sub2=all_sub2[,which(colnames(all_sub2)%in%metadata3$samples.submitter_id)]
dim(all_sub2)
####################################################################################################################################
################################## ANALYSIS PER TISSUE #############################################################################
####################################################################################################################################
...
for (tissue in tissues) {
  ...
  dim(metadata_tissue)
  table(metadata_tissue[,"ancestry"])
  ...
  if (length(metadata_tissue$samples.submitter_id)<20) {
    cumulative_results=rbind(cumulative_results,data.frame("tissue"=tissue, "trait"="ancestry", 
  ...
  for (i in seq(1,100,by=1)) {
    ...
    index = createDataPartition(metadata4[,"ancestry"], p = 0.7, list = FALSE)
    ...
    dim(trainX)
    dim(testX)
    table(metadata_train[,"ancestry"])
    table(metadata_test[,"ancestry"])
    dim(all_sub4)
    ...
    TypeComparison = metadata_train[,"ancestry"]
    TypeString = "['White']"
    TypeComparisonFactor = factor(ifelse(TypeComparison == TypeString, yes = "White", no = "Black"),
    ...
    TypeComparison = metadata_test[,"ancestry"]
    TypeString = "['White']"
    TypeComparisonFactor = factor(ifelse(TypeComparison == TypeString, yes = "White", no = "Black"),
                                   levels = c("White", "Black"))
    table(TypeComparisonFactor)
    ...
    refactoredTrainY = relevel(refactoredTrainY, ref = gsub('([[:punct:]])|\\s+','',"White"))
    refactoredTestY = relevel(refactoredTestY, ref = gsub('([[:punct:]])|\\s+','',"White"))
    ...
    predProbs = as.numeric(predict(mlModel, newdata = testX, type = "prob")[, gsub('([[:punct:]])|\\s+','',"White")])
    fg = predProbs[refactoredTestY == gsub('([[:punct:]])|\\s+','',"White")]
    bg = predProbs[refactoredTestY == "Black"]
  ...
  write.csv(permutation_results,paste("./permutations_ancestry/CSS_core_",sub(" ","_",tissue),"_","ancestry",".csv",sep=""))
  ...
  temp=data.frame(tissue, "ancestry", mean(unlist(permutation_results$AUROC)), margin_of_error_X1,mean(unlist(permutation_results$AUPR)), margin_of_error_X2, mean(unlist(permutation_results$AUPR_random)))
...
write.csv(cumulative_results2,file=paste("./CSS_total_tissues","_","ancestry",".csv",sep=""))
