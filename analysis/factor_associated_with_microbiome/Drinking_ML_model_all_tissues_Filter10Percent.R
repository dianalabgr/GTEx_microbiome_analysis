...
dim(all_sub2)
table(metadata3$drinking_status)
...
for (tissue in tissues) {
  ...
  dim(metadata_tissue)
  table(metadata_tissue[,eval("drinking_status")])
  ...
  if (length(metadata_tissue$samples.submitter_id)<20) {
    cumulative_results=rbind(cumulative_results,data.frame("tissue"=tissue, "trait"="drinking_status", 
  ...
  for (i in seq(1,100,by=1)) {
    ...
    table(metadata_test[,"drinking_status"])
    dim(all_sub4)
  ...
  write.csv(permutation_results,paste("./permutations_drinking_status/CSS_core_",sub(" ","_",tissue),"_","drinking_status",".csv",sep=""))
...
write.csv(cumulative_results2,file=paste("./CSS_total_tissues","_","drinking_status",".csv",sep=""))
