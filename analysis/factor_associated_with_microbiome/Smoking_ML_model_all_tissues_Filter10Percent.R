...
dim(all_sub2)
table(metadata3$smoking_status)
...
for (tissue in tissues) {
  ...
  dim(metadata_tissue)
  table(metadata_tissue[,eval("smoking_status")])
  ...
  if (length(metadata_tissue$samples.submitter_id)<20) {
    cumulative_results=rbind(cumulative_results,data.frame("tissue"=tissue, "trait"="smoking_status", 
  ...
  for (i in seq(1,100,by=1)) {
    ...
    index <- createDataPartition(metadata4[,"smoking_status"], p = 0.7, list = FALSE)
    ...
    dim(trainX)
    dim(testX)
    table(metadata_train[,"smoking_status"])
    table(metadata_test[,"smoking_status"])
    dim(all_sub4)
    ...
    metaSeqObject_CSS  = cumulative_resultscumNorm( metaSeqObject , p=cumNormStatFast(metaSeqObject) )
    ...
    TypeComparison <- metadata_train[,eval("smoking_status")]
    TypeString = "Yes"
    ...
    TypeComparison <- metadata_test[,"smoking_status"]
    TypeString = "Yes"
    TypeComparisonFactor <- factor(ifelse(TypeComparison == TypeString, yes = "Yes", no = "No"),
                                   levels = c("Yes", "No"))
  ...
  write.csv(permutation_results,paste("./permutations_smoking_status/CSS_core_",sub(" ","_",tissue),"_","smoking_status",".csv",sep=""))
  ...
  temp=data.frame(tissue, "smoking_status", mean(unlist(permutation_results$AUROC)), margin_of_error_X1,mean(unlist(permutation_results$AUPR)), margin_of_error_X2, mean(unlist(permutation_results$AUPR_random)))
...
write.csv(cumulative_results2,file=paste("./CSS_total_tissues","_","smoking_status",".csv",sep=""))
