...
#tissues with significant ML models
keep_tissues=c("Muscle","Heart", "Stomach", "Colon",
               "Testis", "Blood", "Liver", "Bladder")

metadata2=metadata2[which(metadata2$tissue_type%in%keep_tissues),]
table(metadata2$tissue_type)
dim(all_sub)
all_sub=all_sub[,which(colnames(all_sub)%in%metadata2$samples.submitter_id)]
dim(all_sub)
metadata2=metadata2[match(colnames(all_sub), metadata2$samples.submitter_id), ]
...
data_healthy_first=read.delim("/mnt/raid1/argis/GTEx/after_access/normal_samples_OtherDatesets/tissues/PRJEB4337/Agamemnon_after_bowtie/final/all/Agamemnon_after_bowtie_species_PRJEB44337_11Tissues.tab")
...
set.seed(50)
cumulative_results_ROC_PR=data.frame(matrix(nrow=length(keep_tissues), ncol=10))
...
rownames(cumulative_results_ROC_PR)=keep_tissues
for (tissue in keep_tissues) {
  ...
  #Check this out https://s3.amazonaws.com/assets.datacamp.com/production/course_6650/slides/chapter2.pdf
    ...
    #Check with living tissues from different dataset
  
    data_normalised_healthyY=metadata_healthy$tissue
    table(data_normalised_healthyY)
    ...
    pr_living=pr.curve(scores.class0 = fg2, scores.class1 = bg2, curve = T, rand.compute=T)
    print(roc_living)
    print(pr_living)
...
write.csv(cumulative_results_ROC_PR,file="./Random_8Tissues_CssOnly_GTEX_results_Filter10percent_Agamemnon_LiivingProject.csv")

#Compare the performances of AUROC and AUPR between the random and not random results 
heart_random=read.csv(file="./ROC_random_shuffle_Heart.csv")
heart_notRandom=read.csv(file="../ROC_shuffle_Heart.csv")

#Calculate Mann-whiteny for AUROC of normal tissues
wilcox.test(heart_random$X4, heart_notRandom$X4)
wilcox.test(heart_random$X5, heart_notRandom$X5)


colon_random=read.csv(file="./ROC_random_shuffle_Colon.csv")
colon_notRandom=read.csv(file="../ROC_shuffle_Colon.csv")

#Calculate Mann-whiteny for AUROC of normal tissues
wilcox.test(colon_random$X4, colon_notRandom$X4)
wilcox.test(colon_random$X5, colon_notRandom$X5)

stomach_random=read.csv(file="./ROC_random_shuffle_Stomach.csv")
stomach_notRandom=read.csv(file="../ROC_shuffle_Stomach.csv")

#Calculate Mann-whiteny for AUROC of normal tissues
wilcox.test(stomach_random$X4, stomach_notRandom$X4)
wilcox.test(stomach_random$X5, stomach_notRandom$X5)


testis_random=read.csv(file="./ROC_random_shuffle_Testis.csv")
testis_notRandom=read.csv(file="../ROC_shuffle_Testis.csv")

#Calculate Mann-whiteny for AUROC of normal tissues
wilcox.test(testis_random$X4, testis_notRandom$X4)
wilcox.test(testis_random$X5, testis_notRandom$X5)


liver_random=read.csv(file="./ROC_random_shuffle_Liver.csv")
liver_notRandom=read.csv(file="../ROC_shuffle_Liver.csv")

#Calculate Mann-whiteny for AUROC of normal tissues
wilcox.test(liver_random$X4, liver_notRandom$X4)
wilcox.test(liver_random$X5, liver_notRandom$X5)


bladder_random=read.csv(file="./ROC_random_shuffle_Bladder.csv")
bladder_notRandom=read.csv(file="../ROC_shuffle_Bladder.csv")

#Calculate Mann-whiteny for AUROC of normal tissues
wilcox.test(bladder_random$X4, bladder_notRandom$X4)
wilcox.test(bladder_random$X5, bladder_notRandom$X5)
