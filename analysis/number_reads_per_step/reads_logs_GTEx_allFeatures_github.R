...
#Reads after finding only the reads that are classified as microbial by pufferfish
...
#Reads that are not aligned in human genome using bowtie2
...
#Create a new dataframe with the mean number of each type of reads and the log(mean)
...
#Create a barplot for each tissue
library(reshape2)
...
#Write the dataframe total_means with the mean value of reads per tissue for each preparation step to a csv file
write.csv(total_means, "Cumulative_table_reads_per_tissue_SF1.csv", row.names=FALSE)


jpeg(file="microbial_per_tissue_all2.jpeg",width=25,height=10,units="in",res=450)
ggplot(total_microbial3,aes(x=tissue, y=log_microbial_reads, fill=mean_microbial)) +
  geom_boxplot(fatten = NULL) +
  theme_bw() + 
  stat_summary(fun.y=mean, geom="point", color="black", aes(group=tissue)) +
  ylab('log10(Microbial Reads)') +
  xlab('') + 
