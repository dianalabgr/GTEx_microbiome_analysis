...
GTEx_all_run = read.delim(file="./agamemnon_after_bowtie_allSamples_species.tab")
...
for (tissue in unique(metadata2$tissue_type)) {
  ...
  se_mean_X1 <- sd(tissue_cumulative2$X1) / sqrt(length(tissue_cumulative2$X1))
    # Calculate the margin of error for a 95% confidence interval
  margin_of_error_X1 <- qt(0.975, df = length(tissue_cumulative2$X1) - 1) * se_mean_X1
  
  # Calculate the standard error of the mean
  se_mean_X2 <- sd(tissue_cumulative2$X2) / sqrt(length(tissue_cumulative2$X2))
  # Calculate the margin of error for a 95% confidence interval
  margin_of_error_X2 <- qt(0.975, df = length(tissue_cumulative2$X2) - 1) * se_mean_X2
...
write.csv(cumulative_results_ROC_PR,file="./CssOnly_GTEX_results_Filter10percent_AllTissues_shuffling.csv")


tissue_size=table(metadata2$tissue_type)
tissue_size=tissue_size[rownames(cumulative_results_ROC_PR)]
#The correlation of AUROC and te number of samples per tissue 
correlation_table=data.frame("tissues"=rownames(cumulative_results_ROC_PR), "AUROC"=cumulative_results_ROC_PR$ROC,
                                              "AUPR"=cumulative_results_ROC_PR$PR, "AUPR_random"=cumulative_results_ROC_PR$PR_random,
                                              "TissuesSize"=as.numeric(tissue_size))
correlation_table$"AUPR/AUPR_random"=correlation_table$AUPR/correlation_table$AUPR_random
#Check for normality of the mean AUROC, mean AUPR and tissues size
ks.test(correlation_table$AUROC,"pnorm")
ks.test(correlation_table$AUPR,"pnorm")
ks.test(correlation_table$"AUPR/AUPR_random","pnorm")
ks.test(correlation_table$TissuesSize,"pnorm")
#Calculate the correlation of the mean AUROC and mean AUPR with te TissuesSize
cor.test(correlation_table$AUROC,correlation_table$TissuesSize,method = c("spearman")) 
cor.test(correlation_table$"AUPR/AUPR_random",correlation_table$TissuesSize,method = c("spearman")) 
